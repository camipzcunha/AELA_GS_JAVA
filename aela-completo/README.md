...
Execute no SQL*Plus ou SQL Developer como SYSDBA:

```sql
-- Criar usuário
CREATE USER aela_user IDENTIFIED BY aela_pass;

-- Conceder permissões
GRANT CONNECT, RESOURCE TO aela_user;
GRANT CREATE TABLE, CREATE SEQUENCE, CREATE SESSION TO aela_user;
GRANT UNLIMITED TABLESPACE TO aela_user;
```

Depois, edite `src/main/resources/application.properties`:

```properties
spring.datasource.url=jdbc:oracle:thin:@//localhost:1521/XEPDB1
spring.datasource.username=aela_user
spring.datasource.password=aela_pass
```

> Para Oracle Cloud (FIAP): substitua o URL pelo Wallet connection string fornecido.
...
git clone https://github.com/seu-grupo/aela-api
cd aela-api
...
java -jar target/aela-api-1.0.0.jar
```
